...
class plgSystemMojoomTemplateSwitcher extends JPlugin
{
	function plgSystemMojoomTemplateSwitcher(& $subject)
	{
	...
	function onAfterRoute()
	{
		...
		$browser1 = strpos(strtolower($_SERVER['HTTP_USER_AGENT']),"mobile");
		$browser2 = strpos($_SERVER['HTTP_USER_AGENT'],"iPad");
		$browser3 = strpos(strtolower($_SERVER['HTTP_USER_AGENT']),"berry");
		if ($browser != '' || $browser1 != '' || $browser2 != '' || $browser3 != '' ) { 
		...
		// do the menu manipulation only if tehe current template is set to mojoom for that get the template name
