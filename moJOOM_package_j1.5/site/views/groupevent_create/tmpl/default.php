...
if($this->Event->id == ""){
$document->setTitle(JText::_('CREATE NEW GROUP EVENT'));
}else {
$document->setTitle(JText::_('EDITING GROUP EVENT '));
}
?>
...
<div class="componentcontent"> 
    ...
    <div class="detail">
	    <?php if($this->eventcreatelimit != 0 ) { ?>
    		<span><?php echo JText::sprintf('SUCCESSFULLY CREATE EVENT', $this->Event_Count, $this->eventcreatelimit );?></span>
        <?php } ?>    	 
		    <div class="detail_1">
			...
			<label for="gender"><?php echo JText::_('TYPE'); ?></label>				 	
                ...
                <input type="radio" name="permission" id="permission-open" value="0" <?php if($this->Event->permission == 0 ) { ?> checked="checked" <?php } ?> /><?php echo JText::_('OPEN EV'); ?>
                <!--<span style="width:60%; float:left;">All site members can join your event and RSVP.</span>-->
                </div>
                <div style="float:left; width:50%;">
                <input type="radio" name="permission" id="permission-private" value="1" <?php if($this->Event->permission == 1 ) { ?> checked="checked" <?php } ?>  /><?php echo JText::_('PRIVATE EV'); ?>
